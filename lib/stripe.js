              ...
              <div className="p-6">
                <div className="max-h-[300px] overflow-y-auto mb-6">
                  {selectedProduct.map((product) => (
                    <div
                      key={`${product.id}-${product.selectedTopping || "default"}`}
                      className="flex items-start mb-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0 last:mb-0"
                    >
                      {/* Show correct image based on topping for Honeycomb */}
                      <div className="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0">
                        <Image
                          src={
                            product.id === 6
                              ? product.img[product.selectedTopping] // Show selected topping image
                      ...
                      <div className="ml-4 flex-grow">
                        <div className="flex justify-between">
                          <h4 className="font-bold text-gray-800">
                            {product.topName}
                            {product.id === 6 && product.selectedTopping && (
                              <span className="text-gray-600 text-sm">
                                {" "}
                                ({product.selectedTopping})
                              </span>
                            )}
                          </h4>
